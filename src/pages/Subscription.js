...
const Subscription = () => {
  ...
  const plans = [
    {
      ...
      price: 'Free',
      features: [
        'Work in existing stores',
        'Manager & courier access',
        'Basic analytics',
        'Email support',
        'Standard themes',
      ],
  ...
  const handleCardSubmit = async () => {
    if (
      ...
      !cardForm.paymentCardExpiryMonth ||
      !cardForm.paymentCardExpiryYear ||
          ...
          >
            {plans.map((plan, index) => (
              ...
              >
                ...
                <h4 style={{ margin: '0 0 12px 0', fontSize: 16, fontWeight: 600 }}>{plan.name}</h4>

                <div style={{ fontSize: 28, fontWeight: 700, marginBottom: 16 }}>
                  {plan.price}
                  {plan.id !== 'BASIC' && <span style={{ fontSize: 14 }}>/month</span>}
                </div>
              ...
              <tbody>
                {invoices.map((inv) => (
                  <tr key={inv.id} style={{ borderBottom: '1px solid var(--color-border)' }}>
                    <td style={{ padding: 16, fontSize: 14, color: 'var(--color-text)' }}>
                      {inv.id}
                    </td>
                    <td style={{ padding: 16, fontSize: 14, color: 'var(--color-text)' }}>
                      {inv.date}
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      <select
                        ...
                        onChange={(e) =>
                          setCardForm((prev) => ({
                            ...prev,
                            paymentCardExpiryMonth: e.target.value,
                          }))
                        }
                        style={{
                          width: '100%',
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      <select
                        ...
                        onChange={(e) =>
                          setCardForm((prev) => ({
                            ...prev,
                            paymentCardExpiryYear: e.target.value,
                          }))
                        }
                        style={{
                          width: '100%',
